...
const login = async (req, res) => {
    ...
    try {
        ...
        const login = await db.collection('infousers').findOne({
...
const register = async (req, res) => {
    ...
    try {
        ...
        const existingUser = await db.collection('infousers').findOne({ correo });
        if (existingUser) {
        ...
        const result = await db.collection('infousers').insertOne(newUser);

        // Crear una entrada en la colección 'userlogin'
        const userLoginData = {
            userId: result.insertedId, // ID del nuevo usuario
            fecha: moment().tz("America/Bogota").format("YYYY-MM-DD HH:mm:ss"), // Fecha actual
            correo,
            contraseña: hashedPassword, // Almacenar la contraseña hasheada
            rol: 'user'
        };

        await db.collection('userlogin').insertOne(userLoginData); // Insertar en la colección userlogin
...
const reclamarCodigo = async (req, res) => {
    ...
    try {
        ...
        const infousersCollection = db.collection('infousers');
        ...
        const usuario = await infousersCollection.findOne({ _id: userObjectId });
        if (!usuario) {
